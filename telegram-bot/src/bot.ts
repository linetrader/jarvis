...
const rawCfg: unknown = JSON.parse(readFileSync(CONFIG_PATH, 'utf8'));
...
const cfg: BotConfig = cfgParse.data;
...
const BRAND_TENANT: Record<string, string> = {
...
const COVERED: ProjectInfo[] = Object.values(PROJECTS);
const COVERED_NAMES: string[] = COVERED.map((p) => p.name);
const FIX_PROJECTS: ProjectInfo[] = COVERED.filter((p) => p.mode === 'fix');
const REPORT_PROJECTS: ProjectInfo[] = COVERED.filter((p) => p.mode === 'report');
if (!COVERED.length) { console.error('[FATAL] config.json 에 projects 가 비어있습니다.'); process.exit(1); }
const DEFAULT_FIX: string = FIX_PROJECTS[0]?.name || COVERED_NAMES[0] || '';
...
const escSh = (s: string): string =>
  String(s).replace(/\\/g, '\\\\').replace(/`/g, '\\`').replace(/\$/g, '\\$').replace(/"/g, '\\"');

const PRIMARY: ProjectInfo = PROJECTS[cfg.primaryProject ?? ''] || FIX_PROJECTS[0] || COVERED[0];
...
// ── telegram helpers ─────────────────────────────────────
async function tg<T = unknown>(method: string, body: Record<string, unknown>): Promise<TelegramApiResponse<T>> {
...
async function send(chatId: string, text: string | number): Promise<void> {
...
async function notifyAdmins(text: string): Promise<void> {
  for (const id of ADMIN) await send(id, text);
}
function typing(chatId: string): () => void {
  const tick = (): void => { tg('sendChatAction', { chat_id: chatId, action: 'typing' }).catch(() => {}); };
  tick();
...
async function downloadPhoto(fileId: string): Promise<string | null> {
  try {
    const r = await tg<TelegramFile>('getFile', { file_id: fileId });
    if (!r.ok) return null;
    ...
    return out;
  } catch (e) { console.error('downloadPhoto', (e as Error).message); return null; }
}
...
// ── 진행 메시지 번역 테이블 ──────────────────────────────────
const I18N: Record<string, I18nLangMap> = {
  checking: {
...
function tr(lang: Lang, key: string, ...args: (string | number | boolean | undefined)[]): string {
  const map = I18N[key];
  if (!map) return key;
...
// ── shell helper (cwd 지정) ──────────────────────────────
function sh(cmd: string, cwd: string = REPO_BASE, timeout: number = 120000): Promise<ShellResult> {
  return new Promise((resolve) => {
    exec(cmd, { cwd, timeout, maxBuffer: 1024 * 1024 * 32, shell: '/bin/zsh' },
      (err, stdout, stderr) => resolve({ ok: !err, code: (err as NodeJS.ErrnoException & { code?: number })?.code ?? 0, stdout: stdout || '', stderr: stderr || '' }));
  });
}

// ── git delta helpers (repo 별) ──────────────────────────
async function statusLines(dir: string): Promise<string[]> {
  const r = await sh('git -c core.quotepath=false status --porcelain', dir);
  return r.stdout.split('\n').filter(Boolean);
}
const linePath = (l: string): string => l.slice(3).replace(/^"|"$/g, '').split(' -> ').pop() ?? '';

// ── triage 파싱 (SCOPE = 담당 프로젝트 중 하나 이상, 쉼표 구분) ──
function parseTriage(out: string): TriageResult {
  const grab = (re: RegExp): string => (out.match(re)?.[1] || '').trim();
...
// ── triage 워커 ──────────────────────────────────────────
const triageQ: TriageQueueItem[] = [];
let triageRunning = false;
let currentTriageItem: TriageQueueItem | null = null;
let issueSeq = 0;

const awaitingApprovalItems: AwaitingItem[] = [];

function enqueueTriage(rawItem: Omit<TriageQueueItem, 'id' | 'summary'>): void {
...
async function pumpTriage(): Promise<void> {
...
function buildPendingSection(): string {
  const lines: string[] = [];
  if (currentFixItem) {
...
function buildTriagePrompt(text: string, imagePath: string | null, brand: string | undefined, lang: Lang = '한국어'): string {
...
async function processTriage(item: TriageQueueItem): Promise<void> {
  ...
  const tail = t.tail ? `\n\n${t.tail}` : '';
  const userIcon: Record<string, string> = { BUG: '🔧', USER_ERROR: '💡', EXPECTED: '✅', NEED_INFO: '🙋' };
...
// ── fix 워커 (직렬, 배포 승인 게이트) ────────────────────
const fixQ: FixQueueItem[] = [];
let fixRunning = false;
let fixStatus: FixStatus = 'idle';
let approvalResolver: ((val: ApprovalDecision) => void) | null = null;
const fixingProjects = new Set<string>();
let currentFixItem: FixQueueItem | null = null;

type RawFixItem = Omit<FixQueueItem, 'id' | 'summary' | 'targets' | 'report'> & {
  id?: number;
  summary?: string;
  targets?: string[];
  report?: string;
...
function enqueueFix(rawItem: RawFixItem): void {
...
async function pumpFix(): Promise<void> {
  ...
  while (fixQ.length) {
    ...
    const cbTargets = item.targets.map((n) => PROJECTS[n]).filter((p): p is ProjectInfo => !!p && p.mode === 'fix');
    let cbBlocked = false;
    ...
    if (!cbBlocked) {
      ...
      catch (e) { console.error('fix err', e); await notifyAdmins(`💥 #${item.id} 수정 작업 오류: ${(e as Error).message}`); }
      finally {
...
function buildFixPrompt(item: FixQueueItem, proj: ProjectInfo): string {
...
async function validateRepo(proj: ProjectInfo, delta: string[]): Promise<{ ok: boolean; reports: string[] }> {
...
function parseQa(out: string): { pass: boolean; verdict: string } {
  const verdict = out.match(/QA:\s*(PASS|FAIL[^\n]*)/i)?.[0] || '(QA 결론 미검출 — FAIL 처리)';
  const pass = /QA:\s*PASS/i.test(out) && !/QA:\s*FAIL/i.test(out);
  return { pass, verdict };
}

async function qaReview(perRepo: PerRepoResult[]): Promise<{ pass: boolean; reports: string[] }> {
  const reports: string[] = [];
  let allPass = true;
  ...
  for (const result of qaResults.filter((r): r is NonNullable<typeof r> => r !== null)) {
...
async function processFix(item: FixQueueItem): Promise<void> {
  const targets = item.targets.map((n) => PROJECTS[n]).filter((p): p is ProjectInfo => !!p && p.mode === 'fix');
  ...
  const fixResults = await Promise.all(targets.map(async (proj): Promise<PerRepoResult | null> => {
  ...
  const perRepo = fixResults.filter((r): r is PerRepoResult => r !== null);
  ...
  fixStatus = 'awaiting';
  const awaitingEntry: AwaitingItem = { id: item.id, summary: item.summary, targets: item.targets, report: item.report };
  ...
  const decision = await new Promise<ApprovalDecision>((resolve) => { approvalResolver = resolve; });
...
async function deploy(proj: ProjectInfo, item: { report?: string }): Promise<void> {
  fixStatus = 'deploying';
...
function parseProjectPrefix(raw: string): { project: string; rest: string } {
  const m = raw.match(PREFIX_RE);
  if (m) return { project: m[1].toLowerCase(), rest: m[2].trim() };
  return { project: DEFAULT_FIX, rest: raw };
}

const railwayProjects = (): ProjectInfo[] => COVERED.filter((x) => (x.railway?.services || []).length > 0);

async function doHelp(chatId: string): Promise<void> {
  await send(chatId,
...
const STATUS_KO: Record<FixStatus, string> = {
  idle: '유휴(작업 없음)',
  preparing: '수정 준비 중',
  qa: 'QA 검증 중',
  awaiting: '배포 승인 대기',
...
const statusKo = (): string => STATUS_KO[fixStatus];

async function doPing(chatId: string): Promise<void> {
...
async function doStatus(chatId: string): Promise<void> {
...
async function doTakeItem(chatId: string, id: number, reporter: string): Promise<void> {
  let item: TriageQueueItem | FixQueueItem | null = null;
  ...
  const triageOrFix = item as TriageQueueItem & FixQueueItem;
...
async function doDropItem(chatId: string, id: number): Promise<void> {
  let removed = false;
  let idx = triageQ.findIndex((i) => i.id === id);
...
async function doApprove(chatId: string): Promise<void> {
...
async function doReject(chatId: string): Promise<void> {
...
async function doLogs(chatId: string, projName: string | undefined, svc: string | undefined, n: string | undefined): Promise<void> {
...
async function doRstatus(chatId: string, projName: string | undefined): Promise<void> {
  const proj = projName ? PROJECTS[projName] : undefined;
  ...
  try {
    const j = JSON.parse(r.stdout) as RailwayStatusResponse;
    const lines: string[] = [];
...
async function doRedeploy(chatId: string, projName: string | undefined, svc: string | undefined, source: boolean): Promise<void> {
  const proj = projName ? PROJECTS[projName] : undefined;
  const svcs = proj?.railway?.services || [];
...
async function doWork(chatId: string, projName: string, task: string, imagePath: string | null, reporter: string): Promise<void> {
...
async function doQuery(chatId: string, question: string): Promise<void> {
...
async function _execAdminTask(chatId: string, proj: ProjectInfo, task: string, imagePath: string | null, conflictHint: string = ''): Promise<void> {
...
async function doAdminTask(chatId: string, projName: string, task: string, imagePath: string | null): Promise<void> {
...
async function doAdminDeploy(chatId: string, projName: string): Promise<void> {
...
async function interpretAdmin(raw: string): Promise<AdminIntentResult> {
  ...
  try { return JSON.parse(m[0]) as AdminIntentResult; } catch { return { intent: 'unknown' }; }
}

function roleOf(msg: TelegramMessage): 'admin' | 'tester' | null {
  ...
  if (role === 'admin') {
    ...
    if (pendingConfirm) {
      ...
      if (ans === 'yes') {
        ...
        else if (c.kind === 'deploy') { await send(chatId, `🚀 [${c.project}] 배포 시작합니다.`); await doAdminDeploy(chatId, c.project ?? DEFAULT_FIX); }
        return;
      }
    ...
    const intent: AdminIntent = it.intent || 'unknown';
    switch (intent) {
      case 'ping': await doPing(chatId); break;
      case 'status': await doStatus(chatId); break;
      case 'help': await doHelp(chatId); break;
      case 'logs': await doLogs(chatId, it.project || DEFAULT_FIX, it.service, String(it.lines ?? '')); break;
      ...
      case 'take_item':
        if (it.id) await doTakeItem(chatId, Number(it.id), reporter);
        else await send(chatId, '처리할 이슈 번호를 알려주세요. 예) "3번 처리해줘"');
        break;
      case 'drop_item':
        if (it.id) await doDropItem(chatId, Number(it.id));
        else await send(chatId, '제거할 이슈 번호를 알려주세요. 예) "3번 취소"');
        break;
      case 'deploy': {
...
async function poll(): Promise<void> {
  try {
    const r = await tg<TelegramUpdate[]>('getUpdates', { offset, timeout: POLL_TIMEOUT, allowed_updates: ['message'] });
  ...
  } catch (e) { console.error('poll', (e as Error).message); await new Promise<void>((res) => setTimeout(res, 3000)); }
  setImmediate(poll);
}
...
(async () => {
  try {
    const me = await tg<{ username?: string; id?: number }>('getMe', {});
    if (me.ok) { BOT_USERNAME = me.result.username || ''; BOT_ID = String(me.result.id || ''); }
  } catch (e) { console.error('getMe 실패', (e as Error).message); }
