...
//
// 담당 프로젝트는 config.json 의 projects 로 결정된다. 같은 bot.mjs 가 config 만 바꿔
// 2개(exchange+hub) 봇이든 3개(+trading) 봇이든 동작한다.
//   - mode: fix    → 트리아지 후 해당 repo 에서 자동수정·검증·승인후 배포
//   - mode: report → 트리아지 후 관리자(루트)에게 상세 보고만(자동수정 X)
//
// 파이프라인 (테스터는 배포서버=Railway 에서 테스트):
//   [테스터 그룹채팅]  버그 리포트(/bug 또는 평문)
//     → 트리아지를 1차 프로젝트(오너) 디렉토리에서 실행(그 CLAUDE.md/담당자 컨텍스트): 배포서버 log(railway) + DB(dbq) + Redis(redisq) 조사
//     → 판별: BUG / USER_ERROR / EXPECTED / NEED_INFO
//       · 오해/정상 → 설명 리포트 발송 (코드 수정 X)
//       · BUG → SCOPE 판정 → fix 파이프라인:
//           코드 수정 → 빌드/타입체크 → QA(각 프로젝트 QA Leader 분배 + 루트 계약 정합) → /approve → 각 프로젝트 deploy-manager(runbook: git push→Railway→상태)
//   [관리자 DM]  수정 diff + QA 결과 수신 → /approve 시에만 배포(git push origin main → Railway)
//     → 평문 = 개발자 직접 작업 ("<project>: ..." 프리픽스, 기본 = 첫 fix 프로젝트)
//
// 배포관리자(승인 후)만 git add/commit/push 수행. 그 외 단계는 commit/push 안 함.
...
const REPO_BASE = join(__dirname, '..');                // makemoney 루트
...
// 테스터 그룹 → 거래소 브랜드 매핑 (1봇이 4브랜드 그룹 커버). 리포트 온 그룹으로 브랜드 자동 판별.
const BRAND_GROUPS = {};
for (const [k, v] of Object.entries(cfg.groupBrands || {})) BRAND_GROUPS[String(k)] = String(v);
// 브랜드 → hub tenantId(브로커ID). 미상은 봇이 브랜드 DB/hub 에서 조회.
const BRAND_TENANT = {
...
// 프로젝트 맵 (repoDir → 절대경로). config.json 의 projects 가 이 봇의 담당 범위.
const PROJECTS = {};
...
if (!COVERED.length) { console.error('[FATAL] config.json 에 projects 가 비어있습니다.'); process.exit(1); }
const DEFAULT_FIX = FIX_PROJECTS[0]?.name || COVERED_NAMES[0];

// 진단 래퍼 (datasource 기반, railway connect -p <projectId> -e production 으로 배포서버 읽기전용 접속)
// 어느 cwd 에서도 동작하도록 절대경로 사용.
...
const DATASOURCES =
  `★ 특정 회원의 거래/주문/잔고/포지션 통합 조회는 이 한 줄이면 끝(빠름, 권장):\n` +
...
// 관리자 전용 — 제한 없음 (관리자 DM 에서만 호출)
const ADMIN_TASK_CMD = cfg.adminTaskCmd || `claude -p --permission-mode bypassPermissions --output-format text`;
// shell 인자 안전 따옴표
const shq = (s) => `'${String(s).replace(/'/g, "'\\''")}'`;
// 셸 큰따옴표("...") 안에 넣을 프롬프트 escape — \ ` $ " 모두 처리 (백틱·$ 미처리 시 명령치환으로 프롬프트 깨짐)
const escSh = (s) => String(s).replace(/\\/g, '\\\\').replace(/`/g, '\\`').replace(/\$/g, '\\$').replace(/"/g, '\\"');
// 트리아지(배포서버 진단)를 맡길 1차 프로젝트(오너) — 테스터가 쓰는 프론트.
// 그 디렉토리에서 claude 실행 → 해당 프로젝트 CLAUDE.md·담당자 컨텍스트가 자동 로드된다.
const PRIMARY = PROJECTS[cfg.primaryProject] || FIX_PROJECTS[0] || COVERED[0];

let offset = 0;
if (existsSync(OFFSET_PATH)) offset = Number(readFileSync(OFFSET_PATH, 'utf8')) || 0;
let BOT_USERNAME = '';   // getMe 로 채움 (그룹 멘션 감지용)
let BOT_ID = '';         // 봇 메시지 답장 감지용
...
async function send(chatId, text) {
  ...
  for (const c of chunks) await tg('sendMessage', { chat_id: chatId, text: c, disable_web_page_preview: true });
}
async function notifyAdmins(text) { for (const id of ADMIN) await send(id, text); }
// 처리 중 "입력 중…" 표시를 4초마다 보내 살아있음을 알림. 반환된 함수로 중지.
...
// 텔레그램 사진 → 로컬 파일로 다운로드, 경로 반환 (실패 시 null)
async function downloadPhoto(fileId) {
  try {
    ...
    return out;
  } catch (e) { console.error('downloadPhoto', e.message); return null; }
}
...
function sh(cmd, cwd = REPO_BASE, timeout = 120000) {
  return new Promise((resolve) => {
    exec(cmd, { cwd, timeout, maxBuffer: 1024 * 1024 * 32, shell: '/bin/zsh' },
...
let issueSeq = 0;  // 이슈 번호 카운터 (봇 재시작 시 초기화)

// 수정 완료 후 아직 배포되지 않은(승인 대기) 이슈 목록. 중복 리포트 감지에 활용.
const awaitingApprovalItems = [];  // { id, summary, targets, report }

function enqueueTriage(item) {
  item.id = ++issueSeq;
  item.summary = (item.text || '').slice(0, 50);
  triageQ.push(item);
...
async function pumpTriage() {
  ...
  while (triageQ.length) {
    ...
    try { await processTriage(currentTriageItem); } catch (e) { console.error('triage err', e); }
...
function buildPendingSection() {
  const lines = [];
  if (currentFixItem) {
    lines.push(`  · #${currentFixItem.id} [수정 진행 중] 대상: ${(currentFixItem.targets || [DEFAULT_FIX]).join(',')} — "${(currentFixItem.report || '').slice(0, 80)}"`);
  }
  for (const item of fixQ) {
    lines.push(`  · #${item.id} [수정 대기] 대상: ${(item.targets || [DEFAULT_FIX]).join(',')} — "${(item.report || '').slice(0, 80)}"`);
  }
...
function buildTriagePrompt(text, imagePath, brand, lang = '한국어') {
  ...
  const brandLine = brand
    ...
    : '';
  return (
    `당신은 지금 ${PRIMARY.name} 프로젝트 디렉토리의 QA 담당(오너)입니다. 이 프로젝트의 CLAUDE.md 규칙·트러블슈팅 절차를 따르세요.\n` +
...
async function processTriage({ id, chatId, reporter, text, imagePath, brand }) {
  ...
  // 1차 프로젝트(오너) 디렉토리에서 실행 → 그 프로젝트 CLAUDE.md·담당자 컨텍스트로 배포서버 진단
  ...
  // 데이터 조회(INFO) — 버그 아님. 본인 데이터 조회 결과만 답하고 종료 (수정 파이프라인 X)
  if (t.verdict === 'INFO') {
  ...
  // 이미 수정 완료 후 배포 대기 중인 동일 이슈 재리포트
  ...
  // 테스터(그룹)에게: 이모지 + AI 요약 + 마무리 (개발 용어 노출 없음)
  const tail = t.tail ? `\n\n${t.tail}` : '';
  const userIcon = { BUG: '🔧', USER_ERROR: '💡', EXPECTED: '✅', NEED_INFO: '🙋' }[t.verdict] || '';
  await send(chatId, `${userIcon ? userIcon + ' ' : ''}${t.summary || '(내용을 확인하고 있어요)'}${tail}`);

  // 관리자에게: 상세
  await notifyAdmins(
    `📨 #${id} 테스터 리포트${brand ? ` [${brand}]` : ''}\n"${text}"\n\n` +
  ...
  // report 대상 → 관리자(루트)에게 보고
  for (const n of reportTargets) {
    await notifyAdmins(
      `🔁 이 버그는 ${n} 소관 — 이 봇의 자동수정 대상이 아님(report). 루트(PM/mission-control)에서 ${n} 으로 분담하세요.\n` +
      `심각도 ${t.severity || 'n/a'}\n원문: "${text}"\n분석: ${t.detail || '(없음)'}`);
  }

  // fix 대상 → 수정 파이프라인
  if (fixTargets.length) {
    enqueueFix({ kind: 'bug', targets: fixTargets, report: text, detail: t.detail, reporter, srcChatId: chatId, severity: t.severity, imagePath, triageId: id });
    // enqueueFix 가 #N 알림을 발송하므로 별도 메시지 불필요
  }
...
let fixStatus = 'idle';        // idle | preparing | awaiting | deploying
let approvalResolver = null;
const fixingProjects = new Set(); // 현재 자동수정 파이프라인이 점유 중인 프로젝트
let currentFixItem = null;

function enqueueFix(item) {
  if (!item.id) item.id = ++issueSeq;
  if (!item.summary) item.summary = (item.report || '').slice(0, 50);
  fixQ.push(item);
  notifyAdmins(`⚙️ #${item.id} 수정 큐 등록 (대상: ${(item.targets || [DEFAULT_FIX]).join(', ')}, 대기 ${fixQ.length}건). 준비되면 diff 와 함께 승인 요청합니다.`);
  pumpFix();
}
async function pumpFix() {
  ...
  while (fixQ.length) {
    ...
    // Cross-bot mutex: prevent two telegram bots from simultaneously editing the same repo.
    // Uses POSIX atomic mkdirSync — fails instantly if another bot holds the lock.
    const cbLocks = [];
    const cbTargets = (item.targets || [DEFAULT_FIX]).map((n) => PROJECTS[n]).filter((p) => p?.mode === 'fix');
    let cbBlocked = false;
    for (const proj of cbTargets) {
      const lp = `/tmp/telegram-bot-fix-${proj.name}.lock`;
      try { mkdirSync(lp); cbLocks.push(lp); }
      catch {
        for (const l of cbLocks) try { rmdirSync(l); } catch {}
        cbLocks.length = 0; cbBlocked = true;
        await notifyAdmins(`⚠️ #${item.id} 다른 봇이 [${proj.name}] 수정 중입니다. 완료 후 재시도하세요.`);
        break;
      }
    }
    if (!cbBlocked) {
      try { await processFix(item); }
      catch (e) { console.error('fix err', e); await notifyAdmins(`💥 #${item.id} 수정 작업 오류: ${e.message}`); }
      finally {
        (item.targets || [DEFAULT_FIX]).forEach((n) => fixingProjects.delete(n));
        for (const l of cbLocks) try { rmdirSync(l); } catch {}
        currentFixItem = null;
      }
    } else {
...
function buildFixPrompt(item, proj) {
  ...
  if (item.kind === 'design') {
    return (
      `참고 디자인 이미지를 바탕으로 UI 를 수정하세요. 이 repo(${proj.name})의 CLAUDE.md 규칙을 준수하세요.\n\n` +
      imgLine +
      `요청: "${item.report || '첨부 이미지의 디자인에 맞춰 해당 화면 UI 를 조정'}"\n\n` +
      `대상 화면을 코드에서 찾아 이미지에 가깝게 수정한 뒤 무엇을 왜 바꿨는지 1~3줄로 요약하세요. 로직은 건드리지 말고 표현/레이아웃 위주로.`);
  }
  if (item.kind === 'manual') return imgLine + item.report;
  return (
    `테스터가 리포트한 버그를 ${proj.name} repo 에서 수정하세요. 변경은 최소화하고 이 repo 의 CLAUDE.md 규칙을 지키세요.\n` +
...
// 변경된 파일들에 대해 repo 별 검증 (apps/<app> → tsc / flutter analyze, 그 외 → skip)
async function validateRepo(proj, delta) {
  const apps = new Set();
  let outsideApps = false;
  for (const f of delta.map(linePath)) {
    const m = f.match(/^apps\/([^/]+)\//);
    if (m) apps.add(m[1]); else outsideApps = true;
  }
  const reports = [];
  let ok = true;
  if (outsideApps && apps.size === 0) reports.push(`⚠️ ${proj.name}: apps 외부 변경 — tsc 건너뜀(수동 확인)`);
  for (const app of apps) {
    const cmd = app === 'mobile' ? 'cd apps/mobile && flutter analyze' : `cd apps/${app} && npx tsc --noEmit`;
    const tr = await sh(cmd, proj.dir, 300000);
    reports.push(`${tr.ok ? '✅' : '❌'} ${proj.name}/${app} ${app === 'mobile' ? 'analyze' : 'tsc'}${tr.ok ? '' : '\n' + (tr.stdout + tr.stderr).slice(-1200)}`);
    ok &&= tr.ok;
  }
  return { ok, reports };
}
...
// QA Leader 검증 — 루트가 조율: 각 repo 는 그 프로젝트 QA Leader(cwd=프로젝트)가 자기 규칙으로 검증,
// 여러 repo 가 걸리면 루트가 프로젝트 간 계약 정합을 추가 확인. (읽기전용, 코드 수정 X)
async function qaReview(perRepo) {
  const reports = [];
  let allPass = true;
  const diffs = {};   // proj.name → diff (재사용)

  // 1) 각 프로젝트 QA Leader 에게 자기 repo diff 검증 위임 (병렬)
  const qaResults = await Promise.all(
    perRepo
      ...
      .map(async ({ proj, tracked }) => {
        ...
        const prompt =
          `당신은 ${proj.name} 프로젝트의 오케스트레이터입니다. Agent 도구로 이 프로젝트의 qa-lead 에이전트를 호출해 검증을 위임하세요.\n` +
        ...
        const esc = escSh(prompt);
        const r = await sh(`${QA_CMD} "${esc}"`, proj.dir, TASK_TIMEOUT);   // cwd=프로젝트 → 그 CLAUDE.md/qa-lead 컨텍스트
        const { pass, verdict } = parseQa(r.stdout || r.stderr || '');
        return { proj, diff, pass, verdict };
      })
  );
  for (const result of qaResults.filter(Boolean)) {
    diffs[result.proj.name] = result.diff;
    allPass = allPass && result.pass;
    reports.push(`🧪 [${result.proj.name}] QA Leader: ${result.verdict}`);
  }

  if (!Object.keys(diffs).length) return { pass: true, reports: ['🧪 QA: diff 없음 — 통과'] };

  // 2) 여러 repo 변경이면 루트가 프로젝트 간 계약 정합 추가 확인
  if (Object.keys(diffs).length > 1) {
    const combined = Object.entries(diffs).map(([n, d]) => `### ${n}\n${d}`).join('\n\n');
    const prompt =
      `당신은 makemoney 루트 오케스트레이터입니다. Agent 도구로 루트 qa-lead 에이전트를 호출해 다중 프로젝트 계약 정합 검증을 위임하세요.\n` +
...
async function processFix(item) {
  const targets = (item.targets || [DEFAULT_FIX]).map((n) => PROJECTS[n]).filter((p) => p && p.mode === 'fix');
  if (!targets.length) { await notifyAdmins('⚠️ 유효한 수정 대상이 없습니다.'); return; }
  ...
  const fixResults = await Promise.all(targets.map(async (proj) => {
    ...
    if (delta.length === 0) { await notifyAdmins(`ℹ️ [${proj.name}] 변경 없음 — 이 repo 는 수정 불필요로 판단.`); return null; }
    const v = await validateRepo(proj, delta);
    for (const line of v.reports) await notifyAdmins(line);
    const tracked = delta.filter((l) => !l.startsWith('??')).map(linePath);
    return { proj, delta, tracked, ok: v.ok };
  }));

  const perRepo = fixResults.filter(Boolean);
  ...
  if (!anyChange) {
    ...
    if (item.srcChatId) await send(item.srcChatId, '확인해 봤는데 별도 조치가 필요 없는 건으로 판단됐어요. 혹시 추가로 알려주실 내용이 있으면 말씀해 주세요.');
    return;
  }
  if (!allTestsOk) {
  ...
  // QA Leader 검증 (빌드 통과 후): 루트 조율 → 각 프로젝트 QA Leader 분배 + 계약 정합
  fixStatus = 'qa';
  await notifyAdmins('🧪 QA: 각 프로젝트 QA Leader 에게 검증 분배 중 (+ 루트 계약 정합)…');
  const qa = await qaReview(perRepo);
  for (const line of qa.reports) await notifyAdmins(line);
  if (!qa.pass) {
  ...
  fixStatus = 'awaiting';
  const awaitingEntry = { id: item.id, summary: item.summary, targets: item.targets || [DEFAULT_FIX], report: item.report || '' };
  ...
  if (idx !== -1) awaitingApprovalItems.splice(idx, 1);
  if (decision === 'approve') {
    await Promise.all(perRepo.map(({ proj }) => deploy(proj, item)));
    if (item.srcChatId) await send(item.srcChatId, '🎉 알려주신 문제가 해결됐어요! 다시 확인해 주시면 감사하겠습니다 😊');
  } else {
    await notifyAdmins('🚫 배포 보류. 워킹트리 변경은 유지됩니다.');
  }
}

// 배포 = 각 프로젝트의 deploy-manager 담당자에게 위임 (cwd=프로젝트 → docs/runbooks/deploy.md·CLAUDE.md 따름)
async function deploy(proj, item) {
  ...
  const prompt =
    `당신은 ${proj.name} 프로젝트의 배포관리자(deploy-manager)입니다. 이 프로젝트의 docs/runbooks/deploy.md 와 CLAUDE.md 절차를 따르세요.\n` +
  ...
  const esc = escSh(prompt);
  const r = await sh(`${DEPLOY_CMD} "${esc}"`, proj.dir, 900000); // FAILED 자동수정 재배포 최대 2회 대응 (빌드 1회 ~5분 × 3 = 15분)
...
function isGreeting(s) {
  const t = (s || '').trim();
  if (t.length < 5) return true;   // 멘션만 하고 내용이 거의 없음
...
function normalize(text) {
  const t = text.trim();
  if (['/approve', '승인', '배포', 'ㅇㅋ', 'ok', 'OK'].includes(t)) return '/approve';
  if (['/reject', '거부', '취소', '거절', 'no'].includes(t)) return '/reject';
  if (['/status', '상태'].includes(t)) return '/status';
  if (['/help', '도움말', '명령'].includes(t)) return '/help';
  return t;
}

// 관리자 평문에서 "<project>: ..." 프리픽스 추출 (기본 = 첫 fix 프로젝트)
const PREFIX_RE = new RegExp(`^(${COVERED_NAMES.join('|')})\\s*[:：]\\s*([\\s\\S]+)`, 'i');
function parseProjectPrefix(raw) {
  const m = raw.match(PREFIX_RE);
  if (m) return { project: m[1].toLowerCase(), rest: m[2].trim() };
  return { project: DEFAULT_FIX, rest: raw };
}

// ── 관리자 액션 헬퍼 (슬래시·자연어 양쪽에서 호출) ──────────
const railwayProjects = () => COVERED.filter((x) => (x.railway?.services || []).length);
async function doHelp(chatId) {
  await send(chatId,
    `🤖 ${BOT_NAME} [관리자] — 담당: ${COVERED_NAMES.join(', ')}\n` +
...
const STATUS_KO = { idle: '유휴(작업 없음)', preparing: '수정 준비 중', qa: 'QA 검증 중', awaiting: '배포 승인 대기', deploying: '배포 중' };
const statusKo = () => STATUS_KO[fixStatus] || fixStatus;
...
async function doStatus(chatId) {
  ...
  if (!currentTriageItem && !currentFixItem && !triageQ.length && !fixQ.length) lines.push('\n(대기 중인 이슈 없음)');
  lines.push(`\n\n"N번 처리해줘" → 해당 이슈를 큐에서 꺼내 관리자 직접 처리\n"N번 취소" → 큐에서 제거`);
  await send(chatId, lines.join(''));
}

// 이슈를 큐에서 꺼내 관리자가 직접 처리
async function doTakeItem(chatId, id, reporter) {
  let item = null;
  let idx = triageQ.findIndex((i) => i.id === id);
  if (idx !== -1) { item = triageQ.splice(idx, 1)[0]; }
  else { idx = fixQ.findIndex((i) => i.id === id); if (idx !== -1) item = fixQ.splice(idx, 1)[0]; }
  if (!item) { await send(chatId, `#${id} 를 대기 큐에서 찾을 수 없습니다. (/status 로 목록 확인)`); return; }
  const task = [item.detail ? `트리아지 분석: ${item.detail}` : '', item.report || item.text || ''].filter(Boolean).join('\n\n');
  const projName = (item.targets?.[0]) || DEFAULT_FIX;
  await send(chatId, `✅ #${id} 큐에서 제거 → 직접 처리합니다.`);
  await doAdminTask(chatId, projName, task, item.imagePath);
}
...
async function doDropItem(chatId, id) {
  let removed = false;
  let idx = triageQ.findIndex((i) => i.id === id);
  if (idx !== -1) { triageQ.splice(idx, 1); removed = true; }
  else { idx = fixQ.findIndex((i) => i.id === id); if (idx !== -1) { fixQ.splice(idx, 1); removed = true; } }
  await send(chatId, removed ? `🗑️ #${id} 큐에서 제거했습니다.` : `#${id} 를 대기 큐에서 찾을 수 없습니다.`);
}
async function doApprove(chatId) {
  if (fixStatus === 'awaiting' && approvalResolver) { const r = approvalResolver; approvalResolver = null; r('approve'); }
  else await send(chatId, `지금은 배포 승인 대기 중인 수정이 없어요 (현재 상태: ${statusKo()}).`);
}
async function doReject(chatId) {
  if (fixStatus === 'awaiting' && approvalResolver) { const r = approvalResolver; approvalResolver = null; r('reject'); }
  else await send(chatId, `지금은 보류할 수정이 없어요 (현재 상태: ${statusKo()}).`);
}
async function doLogs(chatId, projName, svc, n) {
  const proj = PROJECTS[projName];
  if (!proj) { await send(chatId, `로그 볼 프로젝트를 알려주세요: ${COVERED_NAMES.join(', ')}`); return; }
  const svcs = proj.railway?.services || [];
  if (!svcs.length) { await send(chatId, `${proj.name} 은 railway 서비스가 설정돼 있지 않습니다(로컬 운영).`); return; }
  if (!svc || !svcs.includes(svc)) { await send(chatId, `[${proj.name}] 서비스를 골라주세요: ${svcs.join(', ')}`); return; }
...
async function doRstatus(chatId, projName) {
  const proj = PROJECTS[projName];
  if (!proj || !(proj.railway?.services || []).length) { await send(chatId, `배포현황 볼 프로젝트를 알려주세요(railway 사용): ${railwayProjects().map((x) => x.name).join(', ') || '없음'}`); return; }
  ...
  } catch { await send(chatId, (r.stdout || r.stderr).slice(-1500) || '조회 실패'); }
}
async function doRedeploy(chatId, projName, svc, source) {
  const proj = PROJECTS[projName];
  const svcs = proj?.railway?.services || [];
  if (!proj || !svc || !svcs.includes(svc)) { await send(chatId, `재배포할 프로젝트/서비스를 알려주세요. railway: ${railwayProjects().map((x) => x.name).join(', ')}, 서비스: ${svcs.join(', ') || '?'}`); return; }
...
async function doWork(chatId, projName, task, imagePath, reporter) {
  ...
  if (!PROJECTS[project] || PROJECTS[project].mode !== 'fix') { await send(chatId, `자동수정 대상이 아닙니다(가능: ${FIX_PROJECTS.map((x) => x.name).join(', ')}).`); return; }
...
// 배포서버 데이터/현황 읽기전용 조회 (코드 수정 아님). dbq/redisq 래퍼만 사용.
async function doQuery(chatId, question) {
  const lang = detectLang(question);
  await send(chatId, tr(lang, 'queryChecking'));
  const prompt =
    `관리자가 배포서버(production, Railway) 데이터를 조회 요청했습니다:\n"${question}"\n\n` +
...
// 관리자 전용 자유 작업 — Claude Code bypassPermissions, 관리자 DM 에서만 호출
...
async function doAdminTask(chatId, projName, task, imagePath) {
  ...
  if (!proj) { await send(chatId, `대상 프로젝트를 알 수 없습니다: ${COVERED_NAMES.join(', ')}`); return; }

  let conflictHint = '';
  if (fixingProjects.has(proj.name)) {
    // 같은 프로젝트 수정 중 → 현재 변경된 파일 목록을 실시간으로 읽어 충돌 회피 힌트 생성
...
// 관리자 직접 배포 — 승인 게이트 없이 현재 워킹트리 push → Railway
async function doAdminDeploy(chatId, projName) {
  const proj = PROJECTS[projName] || PROJECTS[DEFAULT_FIX];
  if (!proj) { await send(chatId, `배포 대상을 알 수 없습니다: ${COVERED_NAMES.join(', ')}`); return; }
...
function yesNo(s) {
  ...
  if (/^(예|네|응|어|ㅇ|ㅇㅇ|yes|y|ok|오케이|좋아|해|해줘|진행|고고|ㄱㄱ|맞아)\b/.test(t) || ['예','네','응','ㅇㅇ'].includes(t)) return 'yes';
  if (/^(아니|아뇨|아니오|노|no|n|취소|하지마|멈춰|그만|stop)\b/.test(t) || ['아니','노','취소'].includes(t)) return 'no';
  return null;
}

// 자연어 의도 해석 (관리자 자유 문장 → intent JSON)
let pendingConfirm = null;   // {kind:'approve'|'redeploy', project, service, source}
async function interpretAdmin(raw) {
  const svcHint = railwayProjects().map((p) => `${p.name}:[${(p.railway.services || []).join(',')}]`).join(' ') || '없음';
  const prompt =
    `너는 텔레그램 봇 관리자 명령 해석기다. 관리자의 한국어/영어 자유 문장을 아래 intent 중 하나로 분류하고 인자를 뽑아 JSON 한 줄로만 출력해라(다른 말·코드블록 금지).\n` +
    `intent 종류:\n` +
  ...
  try { return JSON.parse(m[0]); } catch { return { intent: 'unknown' }; }
}
...
function roleOf(msg) {
  ...
  if (type === 'private' && ADMIN.has(id)) return 'admin';
  if ((type === 'group' || type === 'supergroup') && GROUPS.has(id)) return 'tester';
  return null;
}
...
async function onMessage(msg) {
  ...
  if (!role) { if (msg.chat.type === 'private') await send(chatId, `⛔ 미인가 (${chatId}).`); return; }

  // ── 관리자(DM) ── 자연어 + 슬래시 둘 다 지원
  if (role === 'admin') {
    // 0) 확인 대기(예/아니오) 우선 처리
    if (pendingConfirm) {
      const ans = yesNo(raw);
      if (ans === 'yes') {
        const c = pendingConfirm; pendingConfirm = null;
        if (c.kind === 'approve') { await send(chatId, '✅ 배포 승인합니다.'); await doApprove(chatId); }
        else if (c.kind === 'redeploy') await doRedeploy(chatId, c.project, c.service, c.source);
        else if (c.kind === 'deploy') { await send(chatId, `🚀 [${c.project}] 배포 시작합니다.`); await doAdminDeploy(chatId, c.project); }
        return;
      }
      if (ans === 'no') { pendingConfirm = null; await send(chatId, '취소했습니다.'); return; }
      pendingConfirm = null;   // 애매하면 취소하고 새 명령으로 해석
    }

    // 예/아니오인데 pendingConfirm 없음 = 봇 재시작으로 컨텍스트 초기화됨
    if (yesNo(raw) !== null) {
      await send(chatId, '⚠️ 확인 대기 중인 요청이 없습니다 (봇 재시작으로 초기화됨). 요청을 다시 보내주세요.');
      return;
    }

    // '/bot <자연어>' = 자연어 프리픽스(그룹 /bot 과 통일). 벗기면 자연어 경로로.
    const botPrefixed = /^\/bot\b/i.test(raw);
    const body = botPrefixed ? raw.replace(/^\/bot(@\w+)?\s*/i, '').trim() : raw;
    if (botPrefixed && !body) { await doHelp(chatId); return; }

    if (!botPrefixed) {
      // 1) 슬래시/한글별칭 빠른 경로
      if (text === '/help') { await doHelp(chatId); return; }
      if (text === '/ping') { await doPing(chatId); return; }
      if (text === '/status') { await doStatus(chatId); return; }
      if (text === '/approve') { await doApprove(chatId); return; }
      if (text === '/reject') { await doReject(chatId); return; }
      if (text.startsWith('/logs')) { const p = raw.split(/\s+/); await doLogs(chatId, p[1], p[2], p[3]); return; }
      if (text.startsWith('/rstatus')) { const p = raw.split(/\s+/); await doRstatus(chatId, p[1]); return; }
      if (text.startsWith('/redeploy')) {
        const p = raw.split(/\s+/);
        if (p[3] !== 'ok') { await send(chatId, `⚠️ 프로덕션 재배포는 확인 필요: /redeploy ${p[1] || '<프로젝트>'} ${p[2] || '<서비스>'} ok`); return; }
        await doRedeploy(chatId, p[1], p[2], p[4] === 'source'); return;
      }
      if (text.startsWith('/')) { await send(chatId, `알 수 없는 명령: ${text}`); return; }

      // 2) 사진 첨부 = 디자인 작업 (자연어 해석 불필요)
      if (photo) {
        const imagePath = await downloadPhoto(photo.file_id);
        if (!imagePath) { await send(chatId, '⚠️ 이미지 다운로드 실패.'); return; }
    ...
    // 3) "<project>: ..." 프리픽스 명시 → 바로 작업 (해석 생략, 빠름)
    if (PREFIX_RE.test(body)) { const { project, rest } = parseProjectPrefix(body); await doWork(chatId, project, rest, null, reporter); return; }

    // 4) 자유 문장 → 자연어 의도 해석
    const lang = detectLang(body);
    await send(chatId, tr(lang, 'understanding'));
    const it = await interpretAdmin(body);
    switch (it.intent || 'unknown') {
      case 'ping': await doPing(chatId); break;
      case 'status': await doStatus(chatId); break;
      case 'help': await doHelp(chatId); break;
      case 'logs': await doLogs(chatId, it.project || DEFAULT_FIX, it.service, it.lines); break;
      case 'rstatus': await doRstatus(chatId, it.project || DEFAULT_FIX); break;
      case 'reject': await doReject(chatId); break;
      case 'approve':
        if (fixStatus === 'awaiting' && approvalResolver) { pendingConfirm = { kind: 'approve' }; await send(chatId, tr(lang, 'confirmApprove')); }
        else await send(chatId, `지금은 배포 승인 대기 중인 수정이 없어요 (현재 상태: ${statusKo()}).`);
        break;
      case 'redeploy': {
        const proj = PROJECTS[it.project]; const svcs = proj?.railway?.services || [];
        if (!proj || !it.service || !svcs.includes(it.service)) { await send(chatId, `재배포 대상이 불명확합니다. railway: ${railwayProjects().map((x) => x.name).join(', ')}, ${proj ? `서비스: ${svcs.join(', ')}` : ''}`); break; }
      ...
      case 'query': await doQuery(chatId, it.task || body); break;
      case 'work': await doAdminTask(chatId, it.project || DEFAULT_FIX, it.task || body, null); break;
      case 'take_item': if (it.id) await doTakeItem(chatId, Number(it.id), reporter); else await send(chatId, '처리할 이슈 번호를 알려주세요. 예) "3번 처리해줘"'); break;
      case 'drop_item': if (it.id) await doDropItem(chatId, Number(it.id)); else await send(chatId, '제거할 이슈 번호를 알려주세요. 예) "3번 취소"'); break;
      case 'deploy': {
      ...
      default: await doAdminTask(chatId, DEFAULT_FIX, body, null); break;
    }
    return;
  }
  ...
  if (role === 'tester') {
    ...
    // /bot 또는 /bug 명령(privacy ON 에서도 전달됨), 멘션(@봇), 봇 메시지 답장 → 작동.
    const mentioned = BOT_USERNAME && new RegExp(`@${escapeRe(BOT_USERNAME)}\\b`, 'i').test(raw);
    const repliedToBot = BOT_ID && String(msg.reply_to_message?.from?.id || '') === BOT_ID;
    let report = null;
    if (/^\/(bot|bug)\b/i.test(raw)) report = raw.replace(/^\/(bot|bug)(@\w+)?\s*/i, '').trim();
    else if (mentioned || repliedToBot) report = raw.replace(new RegExp(`@${escapeRe(BOT_USERNAME)}`, 'ig'), '').trim();
    else if (TRIAGE_ALL && !raw.startsWith('/')) report = raw;
    if (report === null) return;   // /bot·멘션·답장 아니면 무시(잡담 토큰 절약)

    const imagePath = photo ? await downloadPhoto(photo.file_id) : null;
    // /bot 만 치고 내용 없음 → 안내 응답
    if (!imagePath && isGreeting(report)) {
      await send(chatId, `안녕하세요! ${BOT_NAME}입니다 🤖\n\`/bot\` 뒤에 내용을 적어주세요.\n· 불편 사항: /bot 출금 화면에서 수수료가 두 번 빠져요\n· 내 거래 확인: /bot 주문 12345 처리됐나요? (본인 이메일 또는 주문번호 알려주세요)`);
      return;
    }
    if (!report && imagePath) report = '(첨부 스크린샷 참고)';
    ...
    if (r.ok && r.result.length) {
      for (const u of r.result) {
        ...
        if (u.message && (u.message.text || u.message.photo)) {
          try { await onMessage(u.message); } catch (e) { console.error('onMessage', e); }
        }
      }
      writeFileSync(OFFSET_PATH, String(offset));
    }
  } catch (e) { console.error('poll', e.message); await new Promise((res) => setTimeout(res, 3000)); }
  setImmediate(poll);
}
...
// getMe 로 봇 username/id 확보(멘션·답장 감지) 후 폴링 시작
(async () => {
  try {
    const me = await tg('getMe', {});
    if (me.ok) { BOT_USERNAME = me.result.username || ''; BOT_ID = String(me.result.id || ''); }
  } catch (e) { console.error('getMe 실패', e.message); }
